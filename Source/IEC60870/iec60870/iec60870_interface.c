...
void iec101_init(ParameterConfiguration *para)
{
	...
	sParam101.portNo = para->portNo; 					/*串口号 0—2*/
	...
	sParam101.sourceAddr = para->sourceAddr; 			/*链路地址*/
	sParam101.linkAddrSize = para->linkAddrSize;		/*链路地址长度 1 2*/
	sParam101.ASDUCotSize = para->ASDUCotSize;			/*传送原因长度*/
	sParam101.ASDUAddr = para->ASDUAddr;				/*ASDU地址*/
	sParam101.ASDUAddrSize = para->ASDUAddrSize;		/*ASDU地址长度 1 2*/
	sParam101.InfoAddrSize = para->InfoAddrSize;		/*信息体地址长度 2 3*/ //101为2 104为3
...
void iec104_init(ParameterConfiguration *para)
{
	...
	sParam104.sourceAddr = para->sourceAddr; 	/*链路地址*/
	sParam104.linkAddrSize = para->linkAddrSize;	/*链路地址长度 1 2*/
	sParam104.ASDUCotSize = para->ASDUCotSize;	/*传送原因长度*/
	sParam104.ASDUAddr = para->ASDUAddr;		/*ASDU地址*/
	sParam104.ASDUAddrSize = para->ASDUAddrSize;	/*ASDU地址长度 1 2*/
	sParam104.InfoAddrSize = para->InfoAddrSize;	/*信息体地址长度 2 3*/ //101为2 104为3
