...
void db_lnode_flush(db_list_t* list_head ,int num ,void* new_node_data){
	int counter=0;
	db_lnode_t* current;
...
void db_list_delete(db_list_t** list_head ,int num){
	int counter=1;
	db_lnode_t* current;
	db_lnode_t* tmp;
	if((*list_head)==NULL||list_head==NULL){
...
void db_list_destory(db_list_t* list_head){
	db_lnode_t* current;
	db_lnode_t* pos;
	if(list_head==NULL){
		errno=EINVAL;
...
void db_list_travel(db_list_t** list_head ,void(*do_function)(void* )){
	if((*list_head)->limit_size<0 || (*list_head)==NULL){
		errno=EINVAL;
		return ;
	}
	for(int i=0;i<(*list_head)->limit_size;i++){
		(*do_function)(db_list_visit(list_head,i));
	}
}
int db_list_search(
